...
export class Flow<T extends Values> extends React.Component<
  Props<T>,
  State<T>
> {
  ...
  handleSubmit = async (
    event: React.FormEvent<HTMLFormElement> | MouseEvent
  ) => {
    ...
    if (form && form instanceof HTMLFormElement) {
      ...
      // Check if event is a MouseEvent
      if (event instanceof MouseEvent) {
        // Handle MouseEvent specific logic
      } else {
        const hasSubmitter = (
          evt: any
    ...
    try {
      return await this.props.onSubmit({ ...body, ...this.state.values });
    } finally {
        ...
        {nodes.map((node, k) => {
          ...
          return (
            <Node
              ...
              value={values[id]}
              dispatchSubmit={
                this.handleSubmit as unknown as React.FormEvent<HTMLFormElement>
              }
              setValue={(value) =>
                new Promise((resolve) => {
                  this.setState(
